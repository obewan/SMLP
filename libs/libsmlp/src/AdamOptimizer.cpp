...
AdamOptimizer::AdamOptimizer(float learning_rate, float beta1, float beta2)
    : learning_rate_(learning_rate), beta1_(beta1), beta2_(beta2), t_(1), reset_iterations_(1000), epsilon_(1e-8) {}
...
void AdamOptimizer::UpdateWeightsAndBiases(Layer* layer, float learning_rate) {
    ...
    for (size_t i = 0; i < layer->Connections().size(); i++) {
        moving_averages_[i] = beta1_ * moving_averages_[i] + (1 - beta1_) * gradients[i];
        squared_moving_averages_[i] = beta2_ * squared_moving_averages_[i] + (1 - beta2_) * gradients[i] * gradients[i];
        ...
        layer->Connections()[i].SetWeight(layer->Connections()[i].GetWeight() - weight_update);
    }

    for (size_t i = 0; i < layer->Biases().size(); i++) {
        moving_averages_[i] = beta1_ * moving_averages_[i] + (1 - beta1_) * bias_gradients[i];
        squared_moving_averages_[i] = beta2_ * squared_moving_averages_[i] + (1 - beta2_) * bias_gradients[i] * bias_gradients[i];
